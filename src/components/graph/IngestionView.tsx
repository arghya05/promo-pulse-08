    ...
    <div className="space-y-6">
      ...
      <Card className="p-4">
        ...
        <div className="mt-3 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          {FEEDS.map((f) => (
            <div key={f.id} className="flex h-full flex-col rounded-lg border border-border/70 bg-card p-3">
              ...
              <div className="mt-1 text-xs text-muted-foreground">{f.system}</div>
                    ...
                    <ul className="mt-1 space-y-1 text-xs">
                      {stage.rules.map((r) => (
                        <li key={r} className="flex gap-2">
                          <span className={`mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full ${LAYER_STYLES[stage.layer].dot}`} />
      ...
      <Card className="p-4">
        ...
        <div className="mt-3 flex flex-wrap gap-1.5">
          {tables.map((t) => (
            <button
              ...
              className={`metric-value rounded-md border px-2 py-1 text-xs transition ${
                t.table === activeTable
        ...
        <div className="mt-4 space-y-2">
          ...
          {lineage.layers.map((l, i) => (
            <div key={`${l.layer}-${l.object}-${i}`} className="rounded-lg border border-border/70 p-3">
              <div className="flex flex-wrap items-center gap-2">
