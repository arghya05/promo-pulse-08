...
serve(async (req) => {
  ...
  try {
    ...
    // Build transactions query with optional date filter - OPTIMIZED for speed
    ...
    const [
      ...
      supabaseClient.from('stores').select('*').limit(50),
      supabaseClient.from('promotions').select('*').order('created_at', { ascending: false }).limit(55),
      transactionsQuery,
      supabaseClient.from('customers').select('*').limit(150),
      supabaseClient.from('third_party_data').select('*').order('data_date', { ascending: false }).limit(20),
      supabaseClient.from('products').select('*').limit(50),
      supabaseClient.from('marketing_channels').select('*, promotions(promotion_name)').limit(30),
      supabaseClient.from('competitor_data').select('*').order('observation_date', { ascending: false }).limit(20),
      supabaseClient.from('store_performance').select('*, stores(store_name)').order('metric_date', { ascending: false }).limit(50),
      supabaseClient.from('customer_journey').select('*, customers(customer_name), promotions(promotion_name)').order('touchpoint_date', { ascending: false }).limit(50),
      supabaseClient.from('inventory_levels').select('*, stores(store_name)').order('created_at', { ascending: false }).limit(30)
    ]);
